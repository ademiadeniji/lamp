...
class MLP(common.Module):
    def __init__(
        self, shape, linear_probe=False, layers=[512, 512, 512, 512], act="elu", norm="none", **out
    ):
    ...
    def __call__(self, features):
        ...
        for index, unit in enumerate(self._layers):
            x = self.get(f"dense{index}", tfkl.Dense, unit, trainable=(not self._linear_probe))(x)
            x = self.get(f"norm{index}", NormLayer, self._norm, trainable=(not self._linear_probe))(x)
            x = self._act(x)
        if self._linear_probe:
            x = x.reshape(features.shape[:-1] + [x.shape[-1]])
            return self.get("out_probe", DistLayer, self._shape, **self._out)(x)
        x = x.reshape(features.shape[:-1] + [x.shape[-1]])
...
class NormLayer(common.Module, tf.keras.layers.Layer):
    def __init__(self, impl, trainable=True):
        super().__init__()
        self._impl = impl
    ...
    def build(self, input_shape):
        if self._impl == "keras":
            self.layer = tfkl.LayerNormalization(trainable=self._trainable)
            self.layer.build(input_shape)
        elif self._impl == "layer":
            self.scale = self.add_weight("scale", input_shape[-1], tf.float32, "Ones", trainable=self._trainable)
            self.offset = self.add_weight(
                "offset", input_shape[-1], tf.float32, "Zeros", trainable=self._trainable
            )
